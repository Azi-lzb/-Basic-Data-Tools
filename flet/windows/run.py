"""Command-line entry point for the Flet Windows audit edition."""
...
from uos_audit.calculation import ENGINE_DEFAULT, ENGINE_OPTIONS, engine_status, get_calculator
...
def main() -> int:
    parser = argparse.ArgumentParser(description="基础数据审核工具（Windows Flet 版）")
    parser.add_argument("--check-engine", action="store_true", help="检查 Excel/WPS COM 计算引擎")
    parser.add_argument("--flet", action="store_true", help="启动 Windows Flet 工作台（默认）")
    parser.add_argument("--recalculate", type=Path, help="计算并保存指定的 .xlsx 审核副本")
    parser.add_argument("--audit", action="store_true", help="运行 Windows COM 公式审核主链路")
    parser.add_argument("--summary", action="store_true", help="运行 Windows 任意行/固定行汇总")
    ...
    parser.add_argument("--engine", choices=ENGINE_OPTIONS, default=ENGINE_DEFAULT, help="公式计算引擎（默认：Excel COM）")
    ...
    if args.check_engine:
        ...
        available = [name for name, detail in statuses.items() if detail.get("available")]
        if not available:
            parser.error("未检测到可用的 Excel 或 WPS COM：{}".format(statuses))
        print("可用 Windows COM 计算引擎：{}".format("、".join(available)))
        return 0
    if args.recalculate:
