...
def launch_flet(root: Path) -> None:
    ...
    def main(page: "ft.Page") -> None:
        api = WebApi(root)
        api._log("欢迎使用基础数据审核工具（Windows Flet版）。请先选择源数据目录。")
        page.title = "基础数据审核工具（Windows Flet版）"
        ...
        # 打包图标与窗口标题栏图标是两个独立入口。显式指定 .ico，避免
        # Flutter/Flet 运行时在标题栏回退到默认的小鱼图标。
        page.window.icon = str((root / "assets" / "icon_windows.ico").resolve())
        ...
        page.services.extend([union_base_picker, union_join_picker, run_log_export_picker, history_picker])
        ...
        async def choose_history_file(event=None) -> None:
            files = await history_picker.pick_files(
                allow_multiple=False, allowed_extensions=["xlsx"], dialog_title="选择历史审核说明.xlsx"
            )
            if not files:
                return
            try:
                api.update({"historyPath": files[0].path})
                settings_history_path.value = str(api.state["historyPath"])
                settings_error.value = ""
            except Exception as exc:
                settings_error.value = "历史审核说明设置失败：" + str(exc)
            page.update()
        ...
        settings_engine = ft.Dropdown(
            label="公式引擎", value=str(api.state["calculationEngine"]), width=280,
        ...
        settings_history_path = ft.Text(str(api.state.get("historyPath") or ""), size=12, color=TEXT_2, expand=True, selectable=True)
        ...
        general_content = ft.Column([
            ft.Text("公式引擎", size=14, weight=ft.FontWeight.W_700, color=TEXT),
            ft.Row([
                ft.Text("优先顺序", size=13, color=TEXT, width=160),
                settings_engine,
            ], spacing=12),
            ...
            ft.Container(
                content=ft.Column([
                    ft.Text("汇总、历史问题比对和整改状态均读取此文件；默认使用程序随附的历史审核说明。", size=12, color=TEXT_2),
                    ft.Row([settings_history_path, ft.OutlinedButton("选择文件", icon=ft.Icons.FOLDER_OPEN, on_click=choose_history_file)], spacing=10),
                ], spacing=6),
        ...
        def load_settings() -> None:
            ...
            settings_history_path.value = str(api.state.get("historyPath") or "")
