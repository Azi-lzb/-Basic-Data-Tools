...
DEFAULT_FLOWS = (
    ...
    ("汇总核查表校验", 70, "条件格式结果提取", "是", "跳过", "否", "", SOURCE_DIRECTORY_INPUT, "本期审核结果", "提取任意实际填充色发生变化的条件格式项，追加到本期审核结果"),
...
# JSON 友好的行字典，供 Flet 做可视化编辑；历史审核结果工作表不会被读取、
# 重写或传给前端。
...
HISTORY_WORKBOOK_NAME = "历史审核说明.xlsx"
...
def _ensure_history_workbook(history_path: Path, *, remove_legacy_sheets: bool) -> None:
    """Ensure the history workbook contains history only; never rewrite manual columns."""
    from openpyxl import Workbook, load_workbook
    ...
    try:
        if "历史审核结果" not in workbook.sheetnames:
            if "历史审核记录" in workbook.sheetnames:
                workbook["历史审核记录"].title = "历史审核结果"
            else:
                sheet = workbook.active if workbook.active.title == "Sheet" and len(workbook.sheetnames) == 1 else workbook.create_sheet("历史审核结果")
                sheet.title = "历史审核结果"
        ...
        if remove_legacy_sheets:
            for name in (MAPPING_SHEET, FLOW_SHEET, CUSTOM_FEATURE_SHEET, "区域映射", "自定义功能", "外部工作表", "历史审核记录"):
                if name in workbook.sheetnames and name != "历史审核结果":
                    del workbook[name]
...
def initialize_config(config_path: Path) -> Path:
    ...
    if history_path.name == HISTORY_WORKBOOK_NAME and not history_path.exists():
        # Older releases kept this workbook inside data/ and called it
        # config.xlsx.  Move it once to the user-visible application root.
        candidates = (
...
def load_config_editor_data(config_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "flowHeaders": list(FLOW_HEADERS),
        "modules": modules,
        # Flet 设置中心提交时只保存这个列表；同名项会覆盖内置模块的
        # 默认定义。单独返回它，避免界面把只读的有效模块列表误当作
        # 可直接提交的数据。
        "customModules": saved_modules,
