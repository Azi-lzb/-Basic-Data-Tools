"""Native, batch audit pipeline for Windows.

The pipeline has the same file-safety boundary as the rest of the edition:
source workbooks are never modified; all workbook changes happen in
``_审核版.xlsx`` copies.  Excel/WPS COM is used only after openpyxl has
completed copying formula ranges and external sheets.
"""
...
from .conditional_format import (
    extract_simple_conditional_format_issues,
)
...
def _write_summary(path: Path, issues: list[Issue]) -> None:
    ...
    sheet = book.active
    sheet.title = "本期审核结果"
    headers = ("工作簿名", "工作表名", "定位单元格", "错误类型", "校验指标", "描述", "当前值", "对比值", "差值", "规则编号", "历史校验说明", "审核意见")
    sheet.append(headers)
    for item in issues:
        sheet.append((
            Path(item.source_file).name, item.sheet_name, item.target_cell, item.severity,
            item.check_field, item.detail or item.message, item.target_value, item.comparison_value,
            item.difference_value, item.rule_id, item.institution_feedback, item.auditor_opinion,
        ))
        row = sheet.max_row
    ...
    for column, width in {"A": 38, "B": 24, "C": 14, "D": 16, "E": 28, "F": 42, "G": 18, "H": 18, "I": 18, "J": 38, "K": 34, "L": 24}.items():
        ...
        try:
            ...
            if conditional_ranges:
                ...
                if com_conditional_renderer:
                    ...
                    metrics = getattr(calculator, "last_conditional_metrics", None) or {}
                    if metrics.get("mode"):
                        # WPS fallback evaluated rules from OOXML instead of
                        # reading a rendered colour.  Report its summary and any
                        # rules it could not parse, without pretending they were
                        # rendered.
                        say(
                            "条件格式（{}）：触发 {} 个".format(
                                metrics["mode"], metrics.get("issue_count", 0)
                            )
                        )
                        if metrics.get("unsupported_count"):
                            say("WPS 条件规则暂不支持：{} 条规则".format(metrics["unsupported_count"]))
                        unsupported = 0
                    elif metrics:
                        say(
                    ...
                    started = time.monotonic()
                    extracted, unsupported = extract_simple_conditional_format_issues(
                        workbook_path=conditional_path, ranges=conditional_ranges,
                        structure_ranges=definition.structure_ranges, period=source_period, batch_id=batch_id,
                        source_file=source,
                    )
                    conditional_extract_seconds += time.monotonic() - started
                issues.extend(extracted)
                if unsupported:
                    say("条件格式有 {} 条规则未验证（仅支持简单 TRUE/FALSE expression 规则）".format(unsupported))
