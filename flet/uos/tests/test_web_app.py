...
from uos_audit.calculation import ENGINE_LIBREOFFICE, ENGINE_PYTHON, preferred_engine
...
def test_startup_chooses_detected_engine_by_priority(tmp_path: Path, monkeypatch) -> None:
    statuses = {
    ...
    root.mkdir()
    api = WebApi(root)
    assert api.state["calculationEngine"] == ENGINE_LIBREOFFICE


def test_flet_controller_file_selection_and_engine_setting(tmp_path: Path) -> None:
    root = tmp_path / "uos_audit"
    root.mkdir()
    ...
    api.update({"calculationEngine": ENGINE_PYTHON})
    assert api.state["calculationEngine"] == ENGINE_PYTHON
...
def test_flet_controller_dispatches_audit_flow_with_selected_files(tmp_path: Path, monkeypatch) -> None:
    ...
    api.update({
        ...
        "external": str(external_path), "calculationEngine": ENGINE_PYTHON,
    })
    api.refresh_sources()
    ...
    assert received["calculation_engine"] == ENGINE_PYTHON
