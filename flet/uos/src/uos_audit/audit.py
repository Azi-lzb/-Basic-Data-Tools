    ...
    *,
    ...
    if conditional_ranges and not com_conditional_renderer:
        try:
            ...
            # “条件格式结果提取”是一个独立步骤；“失败后处理=跳过”时应只记录
            # 并继续其余审核，而不是让整批因该步骤中止。只有显式“停止”才 raise。
            if conditional_on_failure == "跳过":
                say("已跳过条件格式提取（步骤失败后处理=跳过）：{}".format(exc))
                conditional_ranges = []
            else:
                raise FormulaEngineError(
                    "条件格式提取必须使用 LibreOffice UNO 实际渲染，当前不可用：{}。"
                    "请安装 python3-uno，或为 LibreOffice 提供自带 Python（见 DEPLOY_UOS.md），"
                    "并将该步骤的“失败后处理”改为“跳过”以在缺失时继续。".format(exc)
                ) from exc
    try:
