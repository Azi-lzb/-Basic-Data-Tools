...
- “模块化功能”固定列为 `功能名、功能类型、命名区域名、是否限定工作簿、工作簿关键字、备注`。功能类型统一使用 `检查_命名区域存在性、核对_表结构比对、修改_外部文件添加、修改_公式校验复制、修改_组合联合核查表、汇总_公式校验结果提取、汇总_任意行汇总、汇总_固定行汇总、汇总_汇总表合并`。“检查_命名区域存在性”仅验证配置的每个名称能解析为实际单元格区域，必须由执行流程显式加入；可用逗号/顿号配置多个必需区域。“修改_组合联合核查表”必须作为独立流程，按源文件主名下划线第一段分组，将同一机构的多个工作簿合并为一本，供用户重建命名区域后单独审核。执行流程固定列为 `流程名、顺序、功能名、启用、失败后处理、是否输出结果、输出文件名、处理对象、备注`；失败后处理仅支持“停止/跳过”，每个流程至少有一个启用步骤输出结果。每次流程运行都生成运行日志 `流程名_运行日志_时间戳.xlsx`，一个功能一个 sheet（检查类=命名区域所在工作表与覆盖区域、表结构比对=文件匹配、外部文件添加/公式校验复制=复制记录、校验结果提取=本期问题、合并=机构输出清单、另有模板体检）。“是否输出结果”只对会产出实际文件的功能生效：外部文件添加=阶段副本目录、公式校验复制=审核副本目录、校验结果提取=本期审核结果工作簿、组合联合核查表=每机构一本合并工作簿，统一命名为 `序号_功能名_时间戳`；“输出文件名”列可为某步骤指定名字，填写后该步骤输出文件/目录改为 `输出文件名_时间戳`（默认配置把两个流程的最终结果预填为流程名）；“处理对象”列声明本步骤读哪份数据：留空=源文件，填前序功能名=读该功能产出的副本（如校验结果提取填“公式校验复制”读审核副本、填“外部文件添加”读外部添加副本；任意行/固定行/汇总表合并填“公式校验复制”可对审核副本做汇总），引用的功能名必须排在本步骤之前，主流程校验结果提取预填“公式校验复制”、公式校验复制预填“外部文件添加”；处理对象只认流程中排在前面的外部文件添加/公式校验复制等会产副本的功能；引用不匹配或不产副本的功能（如检查类、核对类）时，自动跳过非副本功能，兜底到最近前一个产副本功能（之前没有则兜底到源数据目录），并在运行日志提示；填“否”时这些中间文件处理完即清理，检查类与表结构比对始终只记入运行日志。命名区域名按“相同或前缀+下划线”匹配；限定为“是”时，工作簿关键字按包含关系匹配模板文件名。任意行/固定行汇总仅输出汇总区域与表头区域的列交集，并按来源工作表名称归并。“汇总表合并”不依赖命名区域，按实际表头相同与否跨工作表合并。工作台“重置配置”将模块和流程恢复默认值，历史审核结果、自定义按钮不受影响。
