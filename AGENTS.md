...
## 界面设计规范（ToDesk 式设置项，适用于 unified 全部前端）

设置中心（及后续新增设置页）遵循以下视觉与交互规范，来源参考 ToDesk 高级设置：

- **行式布局**：每个设置项占一行（`.gen-row`），左边是名称+一句话说明（`gen-row-name`/`gen-row-desc`），
  右边是控件（`gen-row-right`）。一行高度不超过两行文本；说明文字尽量一句话，最多两行。
- **控件选择**：
  - 是/否类开关 → toggle 按钮（`.gen-toggle`，圆角滑块，开启为主题色）；
  - 多选一 → 下拉框（`.gen-select`，最小宽 200px）；
  - 不再使用单选按钮组和长复选框列表。
- **分组**：相关设置放进带边框圆角的分组卡片（`.gen-card`），卡片上方一行小标题
  （`.gen-card-title`）；组与组之间留白 16~18px。
- **文本**：名称 4~10 字；说明只讲“开启/选择后会发生什么”，不写实现细节。
- 路径/文件名类回显使用文件名（不含绝对路径），可点击（链接态）调用
  `bridge.api.open_path()` 用系统默认程序打开；全路径放悬浮提示。
- 布局总则：左侧主导航 + 每个系统一个页面 + 页内操作；主界面执行类按钮
  归属各自系统页面，不放在全局区域；运行记录（日志）为全局共享。

新增或修改设置项、主界面结构时必须遵循本规范；与旧规范（长复选框、单选组、
绝对路径展示）冲突时以本规范为准。
