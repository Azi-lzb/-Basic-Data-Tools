...
class ExcelSession:
    ...
    def __enter__(self) -> "ExcelSession":
        try:
            ...
            raise ExcelUnavailableError(
                "缺少 pywin32，请先执行 pip install -r requirements.txt"
            ) from exc
    ...
    def read_history(self, path: Path, *, sheet_name: str = "问题历史") -> list[Issue]:
        if not path.exists():
            return []
        ...
        try:
            try:
                ...
                # 将旧版“历史审核记录”平稳迁移为“历史审核结果”。
                if sheet_name != "历史审核结果":
                    return []
                try:
                    sheet = workbook.Worksheets("历史审核记录")
                except Exception:
                    return []
            rows = _matrix(sheet.UsedRange.Value2)
            if len(rows) < 2:
