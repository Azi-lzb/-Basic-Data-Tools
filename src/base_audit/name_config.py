...
GUIDE_COMMENTS: dict[str, dict[int, str]] = {
    ...
    FLOW_SHEET: {
        ...
        6: "只对会产出实际文件的功能生效：外部文件添加 / 公式校验复制输出同名阶段副本目录、审核副本目录，校验结果提取输出同名本期审核结果工作簿；填“否”时这些中间文件处理完即清理。检查类与表结构比对始终只记入运行日志，不受此列影响。",
        7: "输出文件名（可选）：仅对“是否输出结果=是”且会产出实际文件的步骤生效；填写后，该步骤的输出文件/目录名从“序号_功能名_时间戳”改为“输出文件名_时间戳”。校验结果提取、任意行/固定行/汇总表合并等最终结果建议填流程名；外部文件添加、公式校验复制的中间阶段副本默认留空。",
