...
class TemplateTests(unittest.TestCase):
    ...
    def test_standard_formula_result_is_parsed(self) -> None:
        parsed = parse_formula_result("软性|贷款余额|123.45|同比增幅较大")
        self.assertEqual(parsed.severity, "核实")
        self.assertEqual(parsed.indicator, "贷款余额")
        self.assertEqual(parsed.value, "123.45")
        self.assertEqual(parsed.difference_value, "123.45")
        self.assertEqual(parsed.detail, "同比增幅较大")
        self.assertEqual(parsed.message, "贷款余额：同比增幅较大")
    ...
    def test_five_part_formula_result_is_split_for_filtering(self) -> None:
        parsed = parse_formula_result(
            "错|305.22|334.39|29.17|基础数据与参考数据不一致"
        )
        self.assertEqual(parsed.severity, "错误")
        self.assertEqual(parsed.indicator, "")
        self.assertEqual(parsed.comparison_value, "305.22")
        self.assertEqual(parsed.reference_value, "334.39")
        self.assertEqual(parsed.difference_value, "29.17")
        self.assertEqual(parsed.value, "29.17")
        self.assertEqual(parsed.detail, "基础数据与参考数据不一致")
        self.assertEqual(parsed.message, "基础数据与参考数据不一致")

    def test_six_part_formula_result_contains_indicator_and_current_value(self) -> None:
        parsed = parse_formula_result(
            "错误|贷款余额|305.22|334.39|-29.17|基础数据与参考数据不一致"
        )
        self.assertEqual(parsed.indicator, "贷款余额")
    ...
    def test_full_width_pipe_and_pipe_in_detail_are_compatible(self) -> None:
        parsed = parse_formula_result("核实｜10｜12｜-2｜同比异常|需人工复核")
        self.assertEqual(parsed.comparison_value, "10")
        self.assertEqual(parsed.reference_value, "12")
        self.assertEqual(parsed.difference_value, "-2")
        self.assertEqual(parsed.detail, "同比异常|需人工复核")

    def test_legacy_two_part_result_is_compatible(self) -> None:
        parsed = parse_formula_result("错误|余额不能为负数")
        self.assertEqual(parsed.severity, "错误")
        self.assertEqual(parsed.message, "余额不能为负数")
