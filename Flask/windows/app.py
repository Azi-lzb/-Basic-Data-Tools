"""Flask 版基础数据审核工具（Windows 外壳）。

复用 Flask/shared 的唯一业务核心（base_audit 后端 + frontend 前端）：
- 前端 index.html 不做修改，由本模块在 ``<head>`` 注入一个 ``bridge.api``
  桥接层，把 ``bridge.api.方法名(...)`` 代理为 ``POST /api/方法名``。
- 后端直接继承 ``base_audit.web_app.WebApi``，文件选择对话框使用 tkinter，窗口控制按钮改为无操作（浏览器窗口由
  用户自己管理）。
- 平台差异（Excel/WPS COM 引擎）封装在 shared/base_audit/excel_com.py，
  本外壳不含任何 Windows 专属业务逻辑。
"""
...
# 开发布局：Flask/windows 与 Flask/shared 平级；打包布局：shared 是本目录子目录。
...
class FlaskApi(WebApi):
    """WebApi 的 Flask 版：文件对话框用 tkinter，窗口控制为无操作。"""
