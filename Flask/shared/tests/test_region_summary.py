...
from base_audit.region_summary import (
    ...
    _header_signature,
    _joins_local_validation_history,
    _metadata_plan,
...
def test_history_columns_only_join_local_validation_sheet() -> None:
    identity = ["来源文件", "来源工作表", "批次", "表单名称", "规则编号", "规则类型", "规则描述", "校验字段"]
    # “本地校验结果”等含全部身份字段的表追加历史三列。
    assert _joins_local_validation_history([*identity, "当前值", "对比值", "差值"])
    # “业务说明”“任务说明”等普通汇总表缺少身份字段，不追加。
    assert not _joins_local_validation_history(["来源文件", "来源工作表", "序号", "机构名称", "说明"])
