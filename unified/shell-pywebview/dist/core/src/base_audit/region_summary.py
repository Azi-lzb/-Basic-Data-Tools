...
def _joins_local_validation_history(base_headers: list[str]) -> bool:
    """历史三列只对含全部身份字段的表（如“本地校验结果”）追加。

    “业务说明”“任务说明”等普通汇总表没有规则编号等身份字段，保持原样输出。
    """
    return all(
        _field_key(name) in {_field_key(value) for value in base_headers}
        for name in LOCAL_VALIDATION_IDENTITY_FIELDS
    )


def _read_local_validation_history(config_path: Path) -> dict[tuple[str, ...], list[str]]:
    """读取配置里「历史本地校验结果审核」sheet，按复合键聚合历史三列。

    返回 {复合键: [历史触发条数, 历史分类说明, 审核结果]}，各字段的多行值用 | 拼接。
    """
    from openpyxl import load_workbook
    from .history import LOCAL_VALIDATION_HISTORY_SHEET, join_local_validation_history
    ...
    try:
        if LOCAL_VALIDATION_HISTORY_SHEET not in workbook.sheetnames:
            return {}
        sheet = workbook[LOCAL_VALIDATION_HISTORY_SHEET]
        rows = [list(row) for row in sheet.iter_rows(values_only=True)]
        if not rows:
            return {}
        headers = [str(value or "").strip() for value in rows[0]]
        history_rows = [list(row) for row in rows[1:] if any(value not in (None, "") for value in row)]
        return join_local_validation_history(headers, history_rows)
    finally:
        workbook.close()
    ...
    *,
    ...
    with ExcelSession(engine_preference) as excel:
        ...
        try:
            ...
            history_lookup = _read_local_validation_history(config_path)
            for feature in summaries:
                ...
                for data_range in data_ranges:
                    ...
                    expected_headers = (
                        [*base_headers, *LOCAL_VALIDATION_HISTORY_FIELDS]
                        if join_history
                        else base_headers
                    )
                        ...
                        try:
                            ...
                            for value_row in values:
                                if any(value not in (None, "") for value in value_row):
                                    ...
                                    if join_history:
                                        key = _composite_key(base_headers, row, LOCAL_VALIDATION_IDENTITY_FIELDS)
                                        history_values = history_lookup.get(key, [""] * len(LOCAL_VALIDATION_HISTORY_FIELDS))
