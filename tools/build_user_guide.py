...
def build_document() -> None:
    ...
    for text in (
        ...
        "确认输出目录。默认跟随源数据目录并使用“审核结果”子目录；点亮书钉后可固定输出目录。",
        "在待处理文件列表中取消本次不需要处理的文件。不会删除源文件。",
        "点击需要执行的流程，例如“汇总核查表校验”或“汇总校验结果说明”。",
        "任务结束后，先查看运行日志确认没有失败文件，再查看问题汇总和审核副本。",
    ):
        add_list_item(doc, text, steps)

    add_heading(doc, "1.1 两个常用流程", 2)
    add_table(
        doc,
        ["流程", "适用场景", "输出"],
        [
            ["汇总核查表校验", "审核同一类报表的多家机构报送文件。", "机构审核副本、本期审核结果、运行日志。"],
            ["汇总校验结果说明", "汇总机构在系统中下载的校验结果及报送说明。", "汇总结果工作簿、运行日志。"],
        ],
        [2100, 3450, 3810],
        keep_together=False,
    )
    ...
    add_list_item(doc, "程序会跳过自身生成的“审核结果、机构审核副本、运行中间副本、测试结果”目录，避免重复处理。", bullet)
    ...
    add_table(
        ...
        [
            ...
            ["流程名_运行日志_时间戳.xlsx", "每次流程运行都会生成，一个功能一个工作表：检查类列出各命名区域所在工作表与覆盖区域，表结构比对列出各文件匹配结果，外部文件添加 / 公式校验复制列出复制记录，校验结果提取列出本期问题，另有模板体检工作表。"],
            ["序号_功能名_时间戳（或 输出文件名_时间戳）", "仅当执行流程相应步骤“是否输出结果=是”时生成：外部文件添加=阶段副本目录，公式校验复制=审核副本目录，校验结果提取=本期审核结果工作簿。填写“输出文件名”列时以该值替换“序号_功能名”作为前缀，默认配置把两个流程的最终结果预填为流程名。"],
        ],
        [3150, 6210],
        keep_together=False,
    )
    add_callout(
        doc,
        "先看运行日志",
        "本期审核结果为 0 不一定代表没有问题。应先确认“运行日志”中没有失败文件，再核对公式区域和模板。",
        caution=True,
    )
    ...
    add_heading(doc, "4.6 config.xlsx 的三张配置表", 2)
    add_table(
        doc,
        ["工作表", "维护内容"],
        [
            ["模块化功能", "定义模块名称、功能类型和命名区域，例如“公式校验复制”作用于“校验区域”。"],
            ["执行流程", "按顺序组合已启用模块；“是否输出结果”只对会产出文件的功能生效（外部文件添加、公式校验复制、校验结果提取），检查类与表结构比对始终记入运行日志。“输出文件名”列（可选）可为某输出步骤指定名字，填写后该步骤输出文件/目录改为“输出文件名_时间戳”。“处理对象”列（可选）声明本步骤读哪份数据：留空=源文件，填前序功能名=读该功能产出的副本（如校验结果提取填“公式校验复制”读审核副本；任意行汇总填“公式校验复制”可对审核副本做汇总；公式校验复制预填“外部文件添加”）。处理对象只认流程中排在前面的外部文件添加/公式校验复制等会产副本的功能；引用不匹配或不产副本的功能（如检查类、核对类）时，自动跳过非副本功能，兜底到最近前一个产副本功能（之前没有则兜底到源数据目录），并在运行日志提示，不会中断流程。"],
            ["自定义按钮", "把一个按钮名称绑定到一个流程名称；“是否显示=是”时显示在工作台。"],
        ],
        [2400, 6960],
    )
    add_callout(
        doc,
        "恢复默认配置",
        "误删默认模块或流程时，可在工作台点击“初始化配置”。该操作只补齐默认配置，不删除历史审核结果和自定义按钮。config.xlsx 三个配置表的表头单元格都带有批注，说明每列怎么填、如何影响功能输出，悬停表头即可查看。",
    )
    ...
    add_heading(doc, "6. 历史审核结果", 1)
    p = doc.add_paragraph("“本期审核结果”和 config.xlsx 中的“历史审核结果”使用同一组表头：")
    set_run_font(p.runs[0])
    add_code(doc, "工作簿名｜工作表名｜定位单元格｜错误类型｜校验指标｜描述｜当前值｜对比值｜差值｜规则编号｜历史校验说明｜审核意见")
    p = doc.add_paragraph("审核人员可将需要长期保留的问题整行复制到“历史审核结果”，填写“历史校验说明”和“审核意见”。下次同规则编号出现时，程序自动带出这两项人工内容。")
    set_run_font(p.runs[0])
    add_callout(
        doc,
        "规则编号",
        "规则编号以工作簿、工作表、公式单元格、校验指标和序号组成。若复制历史记录造成同一规则的序号重复，可在工作台点击“整理历史规则编号”。",
    )
    ...
    add_table(
        ...
        [
            ...
            ["文件被占用", "关闭 Excel 中打开的模板、源文件、配置文件和上次输出结果后重试。"],
            ["某些文件没有结果", "查看“运行日志”（表结构比对、公式校验复制等工作表），确认是否因结构不匹配或工作表缺失而跳过。"],
            ["需要更改一个流程", "在“执行流程”页调整模块、顺序或启用状态；无需修改 Python 程序。"],
        ],
        [2500, 6860],
        keep_together=False,
    )
